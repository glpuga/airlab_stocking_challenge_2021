...
namespace ek_challenger
{
  ...
  TableScannerNode::TableScannerNode() : pnh_{"~"}
  {
    ...
    detections_pub_ = pnh_.advertise<sensor_msgs::Image>("detections", 1, true);
      ...
      ek_challenger::ScanTable::Response &resp)
  {
    ...
    else if (req.command == req.PROCESS_RESULTS)
    {
      ...
      // auto detected_tomato_cans = table_scanner_ptr_->processResultUsingHoughCircles(detections);
      auto detected_tomato_cans = table_scanner_ptr_->processResultUsingContours(detections);

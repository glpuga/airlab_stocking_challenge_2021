...
BT::NodeStatus GripperControlActionNode::tick() {
  ...
  moveit::planning_interface::MoveGroupInterface::Options options{
      move_group_, "/robot_description",
      ros::NodeHandle("/" + moveit_namespace_)};
  auto move_group_ptr =
  ...
  ROS_DEBUG_STREAM("Current state of the gripper joints is "
  ...
  ROS_DEBUG_STREAM("Target state of the gripper joints is "
                   << joint_group_positions[0] << " "
                   << joint_group_positions[1]);
...
void GripperControlActionNode::halt() { _halt_requested = true; }
}  // namespace ek_challenger
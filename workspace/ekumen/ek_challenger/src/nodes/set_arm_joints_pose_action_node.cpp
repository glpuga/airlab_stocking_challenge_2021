...
BT::NodeStatus SetArmsJointPoseActionNode::tick() {
  ...
  moveit::planning_interface::MoveGroupInterface::Options options{
      move_group_, "/robot_description", ros::NodeHandle("/" + moveit_namespace_)};
  auto move_group_ptr =
      std::make_unique<moveit::planning_interface::MoveGroupInterface>(options);
  move_group_ptr->setMaxAccelerationScalingFactor(1.0);
  move_group_ptr->setMaxVelocityScalingFactor(1.0);

  std::vector<double> joint_group_positions;

  ROS_INFO("Getting joint model");
  ...
  ROS_DEBUG_STREAM("Current state of the joints is "
                   << joint_group_positions[0] << " "
                   << joint_group_positions[1]);

  auto to_rad = [](const double deg) { return 3.14159 / 180.0 * deg; };
  ...
  ROS_DEBUG_STREAM("Target state of the joints is "
                   << joint_group_positions[0] << " "
                   << joint_group_positions[1]);
...
void SetArmsJointPoseActionNode::halt() { _halt_requested = true; }
}  // namespace ek_challenger
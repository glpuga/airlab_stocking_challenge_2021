...
namespace ek_challenger
{
  ...
  namespace
  {
    constexpr double cell_width = TomatoCanDimensions::width() * 1.1;
    constexpr double cell_depth = TomatoCanDimensions::depth() * 1.0;
  }; // namespace
      ...
      else
      {
        if (buffer_[linear_index] == CellContent::Obstacle)
        {
          ...
          collision_object_manager_.addBox(
              ...
              f * cell_width, f * TomatoCanDimensions::height(),
              f * cell_depth);
        }
      }
    }
  ...
  void ShelfScanner::clearObstacles()
  {
    ...
    {
      collision_object_manager_.addBox(fmt::format("{}", i),
                                       geometry_msgs::PoseStamped{}, cell_width,
                                       TomatoCanDimensions::height(), cell_depth);
    }

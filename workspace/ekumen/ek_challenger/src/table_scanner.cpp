...
namespace ek_challenger
{
  ...
  std::vector<geometry_msgs::PoseStamped> TableScanner::processResultUsingContours(
      sensor_msgs::Image &detections_image) const
  {
...

    {
      ...
      // fill cans circumferences
      cv::morphologyEx(image, image, cv::MORPH_CLOSE, element, cv::Point(-1, -1), 5);
    }
    {
      cv::Mat element =
      ...
      cv::morphologyEx(image, image, cv::MORPH_OPEN, element, cv::Point(-1, -1), 5);
    }
    ...
    for (size_t i = 0; i < contours.size(); i++)
    {
      ...
      ROS_INFO_STREAM("Detection center and radius: c=" << center << " r=" << radius);
  ...
  std::vector<geometry_msgs::PoseStamped> TableScanner::processResultUsingHoughCircles(
      sensor_msgs::Image &detections_image) const
  {
    // create an opencv image for processing. Remember that opencv
    // uses rows/cols for order, not x/y
    ...
    {
      const auto min_rad = static_cast<int32_t>(TomatoCanDimensions::width() * 0.7 * 0.5 / buffer_delta_);
      const auto max_rad = static_cast<int32_t>(TomatoCanDimensions::width() * 1.3 * 0.5 / buffer_delta_);
    ...
    ROS_INFO_STREAM("Detected contours: " << circles.size());
    ...
    {
      ROS_INFO_STREAM("Detection center and radius: c=" << center);

      geometry_msgs::PoseStamped pose;
      pose.header.frame_id = base_link_frame_;
      pose.pose.position.x = center[0] * buffer_delta_ + table_distance_to_base_;
      pose.pose.position.y = center[1] * buffer_delta_ - table_width_ / 2.0;
      pose.pose.position.z = z_threshold;
      pose.pose.orientation.w = 1;
